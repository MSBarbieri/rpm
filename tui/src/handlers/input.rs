...
use crate::App;
use crate::{event::Key, router::{ActiveBlock, RouteId}};
...
pub fn handler(key: Key, app: &mut App) {
  match key {
    ...
    Key::Esc => {
      app.router.set_current_route_state(Some(ActiveBlock::Empty), Some(ActiveBlock::Library));
    }
    Key::Enter => {
...
fn process_input(app: &mut App, input: String) {
  ...
  // app.push_navigation_stack(RouteId::Search, ActiveBlock::);
}
